#!/bin/bash
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-05 18:19:23 (ywatanabe)"
# File: ./.bin/utils/rename.sh
...
replace_in_files() {
    echo
    echo "Replacing content in files...: $search_pattern -> $replacement"
    find "$target_dir" -type f -not -path "*/\.*" -not -path "*/__pycache__/*" -not -path "*/node_modules/*" -not -path "*/.git/*" -exec grep -l "$search_pattern" {} \; | while read file_path; do
        echo "Processing file: $file_path"
        if $dry_run; then
            grep --color=always -n "$search_pattern" "$file_path" | head -5
            match_count=$(grep -c "$search_pattern" "$file_path")
...
rename_files_and_dirs() {
    echo
    echo "Renaming files and directories...: $search_pattern -> $replacement"
    find "$target_dir" -depth -not -path "*/\.*" -not -path "*/__pycache__/*" -not -path "*/node_modules/*" -not -path "*/.git/*" | while read item_path; do
        dir_part=$(dirname "$item_path")
        base_part=$(basename "$item_path")
